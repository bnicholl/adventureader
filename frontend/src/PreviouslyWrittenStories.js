...
function PreviouslyGeneratedStories() {
  ...
  useEffect(() => {
    console.log('book_objects')
    // I may pass username as a parameter as that will load immideately when the page renders, which would like  a lil nicer
    if (auth.isAuthenticated){
      ...
      const fetchDataFromDynamoDB = async () => {
        ...
        try {
          ...
          for (let i = 0; i < nine_most_recent_books.length; i++) {
            ...
            if (nine_most_recent_books[i][`error`] && nine_most_recent_books[i][`error`] != 'none' && !allBooksRef.current.includes(nine_most_recent_books[i].book_id) && times_we_query_books.current > 0){
              console.log('sjndajsknlkndsk')
              toast.info('There was an error generating your story. No credits will be subtracted.',{
              autoClose: 150000});
            ...
            if (!booksRef.current.includes(nine_most_recent_books[i].book_id) && nine_most_recent_books[i][`page ${pages_in_book - 1}`] && nine_most_recent_books[i][`error`] === 'none'){
      ...
      const interval = setInterval(fetchDataFromDynamoDB, 10000); // Adjust the interval (e.g., 10000ms = 10 seconds)
...
export default PreviouslyGeneratedStories;
